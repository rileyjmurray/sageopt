...
The equality constraint in this problem creates an unncessary challenge in solution recovery. Since we usually want to recover optimal solutions, we reformulate the problem after substituting :math:`P \leftarrow 70.7107 A_3 / (A_1 + A_3)`, and clearing the denominator :math:`(A_1 + A_3)` from constraints which involved :math:`P`. ::
