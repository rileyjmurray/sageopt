...
def primal_dual_vals(f, ell):
    # primal
    prob = sage_sigs.sage_primal(f, ell=ell)
    status, value = prob.solve(solver='ECOS', verbose=False)
    prim = value
    # dual
    prob = sage_sigs.sage_dual(f, ell=ell)
    status, value = prob.solve(solver='ECOS', verbose=False)
    dual = value
...
def constrained_primal_dual_vals(f, gts, eqs, p, q, ell, AbK, solver='ECOS'):
    # primal
    prob = sage_sigs.constrained_sage_primal(f, gts, eqs, p, q, ell, AbK)
    status, value = prob.solve(solver=solver, verbose=False)
    prim = value
    # dual
    prob = sage_sigs.constrained_sage_dual(f, gts, eqs, p, q, ell, AbK)
    status, value = prob.solve(solver=solver, verbose=False)
    dual = value
    return [prim, dual], prob
...
class TestSAGERelaxations(unittest.TestCase):
    ...
    def test_unconstrained_sage_1(self):
        ...
        optsols, gaps = sage_sigs.dual_solution_recovery(dual)
        assert np.min(gaps) < 1e-6
    ...
    def test_unconstrained_sage_2(self):
        ...
        _, gaps = sage_sigs.dual_solution_recovery(dual)
        assert np.min(gaps) < 1e-6
    ...
    def test_sage_multiplier_search(self):
        ...
        val0 = res0[1]
        assert val0 == -np.inf
        prob1 = sage_sigs.sage_primal(s, ell=1)
    ...
    def test_constrained_sage_1(self):
        ...
        sols, gaps = sage_sigs.dual_solution_recovery(dual, ineq_tol=1e-7)
        assert np.min(np.abs(gaps)) / abs(dual.value) < 1e-4
    ...
    def test_generalized_constrained_sage_2(self):
        ...
        if cl.Mosek.is_installed():
            ...
            _, gaps = sage_sigs.dual_solution_recovery(dual, ineq_tol=0)
            assert np.min(gaps) / dual.value < 1e-2
        else:
